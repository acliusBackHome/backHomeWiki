...
4.2.6.如果i为'\^'，如果l为0切r<0，即可抛出一个“0的负数幂”的错误异常；如果l<0且分数化的r可写为整数 N+2\^(-n)，n为正整数，则可以抛出一个“负数底的偶开根”错误异常，当然如果引入i\^2=-1，这是可以计算的，但是我们这里我们认为不妥；否则使用`math.h`的pow函数计算mid=pow(l,r)，压mid入实数栈。

4.2.7.如果i不和上述情形，即可抛出一个“非法运算符”的错误异常。
...
为了偏向书写习惯，我们将形如(1+ 2)带空格的表达式认为是正确的，但是在许多程序设计语言中这样的表达式也普遍认为是正确的。
